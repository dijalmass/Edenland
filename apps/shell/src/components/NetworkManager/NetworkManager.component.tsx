...
export function NetworkManager() {
  ...
  const onConnect = async (data: PasswordForm) => {
    if (selectedNetwork) {
      ...
      // O hook já trata o toast de erro/sucesso
  ...
  return (
    <>
      {/* Ícone Flutuante na barra superior */}
      ...
      {/* Settings Card - Centralizado como modal */}
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none p-4">
          {/* Backdrop */}
          <div 
            className="absolute inset-0 bg-background/40 backdrop-blur-md pointer-events-auto animate-in fade-in duration-300"
            onClick={toggleOpen}
          />
          
          {/* Card Principal - Aumentado para w-96 */}
          <div className="relative w-full max-w-md max-h-[85vh] flex flex-col rounded-3xl bg-background/95 backdrop-blur-3xl border border-muted/50 shadow-[0_20px_50px_rgba(0,0,0,0.3)] p-6 animate-in fade-in zoom-in-95 duration-300 pointer-events-auto overflow-hidden">
            ...
            {error && (
              <div className="mb-6 px-4 py-3 bg-destructive/10 text-destructive text-xs rounded-xl border border-destructive/20 flex items-start gap-3 animate-in slide-in-from-top-2">
                ...
                networks.length > 0 ? (
                  networks.map((net, i) => (
                    <div key={i} className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            <div className="flex gap-2 mt-0.5">
                              {net.connected && (
                                <span className="text-[9px] text-primary uppercase font-black tracking-wider px-2 py-0.5 bg-primary/10 rounded-full border border-primary/20 flex items-center gap-1">
                      ...
                      {/* Password Input Inline com Hook Form e Botão de Olho */}
                      {selectedNetwork?.ssid === net.ssid && !net.connected && (
                        <form 
                          onSubmit={handleSubmit(onConnect)} 
                          className="flex flex-col gap-3 p-4 bg-muted/30 rounded-2xl mt-2 mb-3 border border-muted/50 animate-in slide-in-from-top-2 duration-300"
                        >
                          ...
                          <div className="flex gap-2">
                            ...
                            <button 
                              type="submit"
                              disabled={isLoading}
                              className="flex-[2] bg-primary text-primary-foreground py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:brightness-110 transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-primary/25 active:scale-[0.98]"
                            >
